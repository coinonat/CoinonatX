...
void Shutdown()
{
    ...
    RenameThread("CoinonatX-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: coinonatx.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: coinonatxd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeCoinonatXamount=<n> " + _("Keep N CoinonatX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CoinonatX is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CoinonatX is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CoinonatX version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "CoinonatX server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CoinonatX") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart CoinonatX to complete") << "\n";
    ...
    nAnonymizeCoinonatXAmount = GetArg("-anonymizeCoinonatXamount", 0);
    if(nAnonymizeCoinonatXAmount > 999999) nAnonymizeCoinonatXAmount = 999999;
    if(nAnonymizeCoinonatXAmount < 2) nAnonymizeCoinonatXAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeCoinonatXAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize CoinonatX Amount %d\n", nAnonymizeCoinonatXAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1CoinonatX+1000 == (.1CoinonatX+100)*10
       10CoinonatX+10000 == (1CoinonatX+1000)*10
    */
