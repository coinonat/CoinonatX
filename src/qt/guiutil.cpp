...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("CoinonatX"))
        return false;
...
{
    // Convert CoinonatX:// to CoinonatX:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("CoinonatX://"))
    {
        uri.replace(0, 12, "CoinonatX:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CoinonatX.lnk";
}
...
{
    return GetAutostartDir() / "CoinonatX.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CoinonatX-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  CoinonatX-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CoinonatX-Qt"));
