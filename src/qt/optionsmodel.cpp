...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCoinonatXAmount"))
        settings.setValue("nAnonymizeCoinonatXAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeCoinonatXAmount = settings.value("nAnonymizeCoinonatXAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeCoinonatXAmount"))
        SoftSetArg("-anonymizeCoinonatXamount", settings.value("nAnonymizeCoinonatXAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeCoinonatXAmount:
            nAnonymizeCoinonatXAmount = value.toInt();
            settings.setValue("nAnonymizeCoinonatXAmount", nAnonymizeCoinonatXAmount);
            emit anonymizeCoinonatXAmountChanged(nAnonymizeCoinonatXAmount);
            break;
        default:
            break;
        }
